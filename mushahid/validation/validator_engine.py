...
async def validate_and_repair_chat_reply(
    *,
    ...
    try:
        ...
        decision = BackendValidationDecision(approved=False, status="denied", score=None, issues=[])
    ...
    try:
        ...
        second_decision = BackendValidationDecision(approved=False, status="denied", score=None, issues=[])

    telemetry.total_latency_ms = (time.perf_counter() - start_time) * 1000

    if second_decision.status == "denied":
